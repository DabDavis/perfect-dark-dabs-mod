...
f32 playerGetCutsceneBodyAlphaFrac(struct prop *prop)
{
	...
	dx = player->cam_pos.x - player->bond2.unk10.x;
	dy = player->cam_pos.y - player->bond2.unk10.y;
	dz = player->cam_pos.z - player->bond2.unk10.z;
...
static const struct thirdpersontether g_ThirdPersonTethers[] = {
	{ 0,                  0     }, // MODTETHER_OFF, never read
	{ 60 * M_PI / 180.0f, 0.02f }, // MODTETHER_LOOSE
	{ 45 * M_PI / 180.0f, 0.05f }, // MODTETHER_NORMAL
	{ 30 * M_PI / 180.0f, 0.12f }, // MODTETHER_TIGHT
};
 ...
 * view to follow it down (third-person.md, "Height has a usable range").
 *
...
static void playerTetherCamera(struct player *player, struct coord *eye, struct coord *offset)
{
	...
	if (g_ModOptions.camtether <= MODTETHER_OFF || g_ModOptions.camtether > MODTETHER_MAX) {
		return;
	}

	tether = &g_ThirdPersonTethers[g_ModOptions.camtether];
...
static void playerPullBackCamera(struct coord *campos)
{
	...
	if (!playerIsThirdPerson(player)) {
		// A spell on the eye - aiming, or the camera off - restarts the tether
		// behind the aim, the same as the first frame of third person does,
		// and the body facing the aim, which is where first person left it.
	...
	playerTetherCamera(player, &player->bond2.unk10, &offset);
 ...
 *
 * A sidestep rolls the picture into the direction of travel, a look up or
 * down leans the camera a little further that way for as long as the view
 * is moving, and a walk bobs the eye up and down with the steps. The roll
 * and the lean chase their target rather than snapping to it, so a tap of
 * the strafe key is a nod and not a jolt, and both come back to level on
 * their own when the input stops.
 *
 * The bob is Quake's: a sine on a fixed cycle, six tenths of a second per
 * step, whose height follows the ground speed, so it fades in over the
 * first strides and out over the last, with the height itself eased so a
 * wall does not stop it dead. Quake lifts the eye more than it drops it,
 * three parts up to seven of swing, and that is kept - a bob that dipped as
 * far as it rose read as the floor moving rather than the walker. The speed
 * is the distance the player actually covered this tick, not the stick, so
 * walking into a wall does not bob, and a fall does not either.
 *
 ...
 * the look settles, and the bob is a few centimetres straight up. The gun
 * is drawn in screen space and comes with the picture.
 *
	...
	if (scale > 0 && !player->isdead && player->bondmovemode == MOVEMODE_WALK) {
		...
		if (!player->isfalling && g_Vars.lvupdate60freal > 0) {
			...
			if (speed < CAMTILT_TELEPORT) {
				...
				bobtarget = speed * CAMTILT_BOB_UNITS * scale;
			}
		}
	}
...
Gfx *playerRenderShield(Gfx *gdl)
{
	...
	if (g_Vars.currentplayer->shieldshowtime >= 0) {
		...
		shieldColourGet(SHIELDCOLOUR_PLAYER, shield, &red, &green, &blue);
