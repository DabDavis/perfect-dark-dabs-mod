...
- **Crash reports** — [crash-reports.md](CLAUDE-notes/crash-reports.md): `sysFatalError()` writes one before the dialog goes up (build, mod, the `[Mod]` settings live rather than from the file, and the last 200 log lines from a **ring** in crashreport.c, because pd.log only exists under `--log`); the dialog's *Send report to Dab* button and *Crash Reports* on the Perfect Menu are the only two things that send one, and the note is typed on the page because a message box cannot take text; `POST /crash` on pdghostd stores it as a file nothing serves back
