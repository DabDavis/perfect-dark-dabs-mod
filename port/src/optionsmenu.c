...
static const struct modpreset g_ModPresets[] = {
	...
	{ "Dab's Settings",   1,    MODROLL_EVERYONE, true,  true,   MODTILT_NORMAL, false, true, 128,    0,    64,    false, false, true,  true,  true,   MODENHANCE_2X, MODVIVID_LIGHT, MODBLACK_LIGHT, true, MODGHOST_OFF,    true,   true,    true,      true,      false, 20 },
...
/**
 * Disable Fog: no level's fog at all, each drawn out to its far side
 * (modIsFogDisabled()). Live: the far plane follows the next frame.
 */
static MenuItemHandlerResult menuhandlerModDisableFog(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GET:
