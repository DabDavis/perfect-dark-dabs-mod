 ...
 *
 * The shape of it: romdataFileLoad() asks xblaStageWants() for every file
 * slot it fills from the ROM, and for a `bgdata/bg_*.seg` while a level that
 * wants the release's rooms is loading, xblaStageLoad() reads the release's
 * copy out of the package, checks that it is the format this port can read,
 * corrects what it knows to be wrong in it, and hands the buffer over. From
 * there bg.c reads it the way it reads the ROM's.
 */
...
static s32 frozen;         // the decision for the running level
static s32 servedFileNum;  // the slot holding the release's copy, or 0
...
// under a running game, so once is enough, and it saves reading a file out
// of the package at every one of the hundred or so accesses a level makes.
...
void xblaStageLevelReset(void)
{
	...
	frozen = optEnabled && xblaMeshGetEnabled();

	// The last level's file goes, so that the next look at that slot - or at
	// any other level's - decides afresh under the new setting.
	if (servedFileNum) {
		romdataFileFree(servedFileNum);
		servedFileNum = 0;
	}
}
...
{
	const s32 want = optEnabled && xblaMeshGetEnabled();

	return STAGE_IS_LEVEL(mainGetStageNum()) && want != frozen;
}
...
s32 xblaStageWants(s32 fileNum, const char *name)
{
	...
	if (fileNum > 0 && fileNum < XBLASTAGE_MAXFILEID && (refused[fileNum >> 3] & (1 << (fileNum & 7)))) {
		return 0;
	}

	if (strncmp(name, "bgdata/bg_", 10) != 0) {
		return 0;
	}

	len = strlen(name);

	return len > 4 && strcmp(name + len - 4, ".seg") == 0;
}

void xblaStageReleased(s32 fileNum)
{
	if (servedFileNum == fileNum) {
		servedFileNum = 0;
	}
}
...
{
	return servedFileNum != 0 && g_StageIndex >= 0
		&& g_Stages[g_StageIndex].bgfileid == servedFileNum;
}
 ...
 * Returns 0 when this is not a file to hand to the game.
 */
static s32 xblaStageFix(u8 *data, u32 len, const char *name)
{
	u32 inflated;
	u32 section1;
	u32 primstored;
	...
	sysLogPrintf(LOG_NOTE, "xblastage: %s from the release: %u bytes, %u rooms, %u vertex loads sized, %u stale blocks emptied",
...
u8 *xblaStageLoad(s32 fileNum, const char *name, u32 *outSize)
{
	...
	// Ready-only: the file slot is not the place to take a 250MB archive
	// apart. The meshes' switch did that, or will, and the level after it
	// has the rooms like anybody else's.
	...
	if (len > XBLASTAGE_MAXFILE || !xblaStageFix(raw, len, name)) {
		free(raw);

		if (fileNum < XBLASTAGE_MAXFILEID) {
	...
	if (servedFileNum && servedFileNum != fileNum) {
		romdataFileFree(servedFileNum);
	}

	servedFileNum = fileNum;
	*outSize = len;
...
void xblaStageSetEnabled(s32 enabled) { }
void xblaStageLevelReset(void) { }
s32 xblaStageWants(s32 fileNum, const char *name) { return 0; }
u8 *xblaStageLoad(s32 fileNum, const char *name, u32 *outSize) { return NULL; }
void xblaStageReleased(s32 fileNum) { }
s32 xblaStageIsRelease(void) { return 0; }
s32 xblaStagePending(void) { return 0; }
void xblaStageSetVerbose(s32 verbose) { }
