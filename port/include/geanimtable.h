 ...
 * offsets into the `animation_data` segment - the same records animConvert()
 * reads. The segment's ROM address is GEANIM_BASE: `idle` is id 0 at offset
 * 0x1c and the intro's own list has it at 0x28e99c.
 *
 * Generated from the GoldenEye decompilation, `src/game/initanitable.c` for the
 * order and `assets/animationtable_data.h` for each one's offset; the reference
 * copy is tools/geconvert/geanimtable.py and the two must agree.
 *
 * The three of `animation_table_ptrs2[]` - an aircraft's own - are left out:
 * an id means one of those when the AI list belongs to a vehicle prop, and the
 * conversion drops those commands.
 */
#ifndef GEANIMTABLE_H
#define GEANIMTABLE_H

#define GEANIM_BASE 0x28e980

static const struct { const char *name; uint32_t at; } g_GeAnims[] = {
	{ "idle", 0x0001c },  // 0
