...
| Increase Poly Models | Characters, weapons and props drawn with more triangles over a smooth surface fitted through the model's own vertices, so a six-sided arm reads as round. Only what the model's own mesh says is curved bends: boxes keep their edges, and lit surfaces the game never built a model for, like the Institute lobby's glass table top, stay flat. Each edge is drawn with only as many pieces as its size on screen can show, so a figure across the arena costs nothing and one filling the screen gets the full level. Light, Normal or Heavy; forces Model LOD off while on |
| Model LOD | The game's own swap to low-detail bodies at a distance. Off keeps the full model at any range |
| Enhance Textures | The game's own textures scaled up two or four times over as they load, resampled through a curve rather than the GPU's straight blend. Nothing is invented, but a 32-texel wall stops being a grid of soft blobs. Textures that are themselves a dot pattern, like a halftone portrait or a screen of tiny text, are left alone. 2x by default; 4x costs four times the video memory |
