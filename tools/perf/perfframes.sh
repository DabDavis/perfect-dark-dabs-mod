#!/bin/bash
# perfframes.sh BIN LABEL [FRAMES] [STAGE] [SIMS] — the same match as perfrun.sh,
...
BIN=$1; LABEL=$2; FRAMES=${3:-2400}; STAGE=${4:-0x32}; SIMS=${5:-80}
...
s=[l for l in open(sys.argv[2]) if 'tris clipped' in l]
if s:
    m=[[int(x) for x in re.findall(r'tris clipped (\d+), culled (\d+), drawn (\d+)',l)[0]] for l in s]
    n=len(m); print(f"  clipped {sum(x[0] for x in m)/n:.0f}, culled {sum(x[1] for x in m)/n:.0f}, drawn {sum(x[2] for x in m)/n:.0f} per sampled frame")
PY
